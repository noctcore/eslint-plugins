...
- [ ] If a rule was added or removed: `EXPECTED_RULE_COUNT` in `site/scripts/parity.test.ts` is updated, the package README table has the row, and the preset either lists the rule at `error` or names it in `OMITTED_FROM_PRESETS` with a reason.
