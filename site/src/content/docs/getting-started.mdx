...
| Add | When |
| --- | --- |
| [`code-quality`](/eslint-plugins/packages/code-quality/), [`async-safety`](/eslint-plugins/packages/async-safety/), [`contracts`](/eslint-plugins/packages/contracts/) | Always: the starter set |
| [`security`](/eslint-plugins/packages/security/) | You run server code (shell, HTTP, redirects, HTML rendering) |
| [`react`](/eslint-plugins/packages/react/) | You write React components |
| [`rsc`](/eslint-plugins/packages/rsc/) | You use the Next.js App Router / React Server Components |
| [`prisma`](/eslint-plugins/packages/prisma/) | You use Prisma (several rules need your tenant models) |
| [`llm`](/eslint-plugins/packages/llm/) | You call an LLM SDK |
| [`observability`](/eslint-plugins/packages/observability/) | You log through a structured logger |
| [`architecture`](/eslint-plugins/packages/architecture/), [`monorepo`](/eslint-plugins/packages/monorepo/) | You want folder and package boundaries enforced (`monorepo` needs your scope) |
...
The steps below take one plugin at a time and explain each piece.
