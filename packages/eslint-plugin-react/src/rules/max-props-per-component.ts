 ...
 * `TSMethodSignature`): members inherited via an interface `extends` clause or an
 * intersection type are deliberately NOT counted, so composing a shared base
 * contract stays free while widening the local surface does not.
 *
 * Layout-independent: it keys off the `*Props` naming convention and inspects any
 * matching interface/type-literal, wherever the file lives.
 */
...
export const maxPropsPerComponentRule = createRule<RuleOptions, MessageIds>({
  ...
  meta: {
    ...
    docs: {
      description:
        'A `*Props` interface/type-literal may declare at most `max` (default 12) local members. `extends` clauses are not counted.',
    },
    schema: [optionSchema],
    messages: {
      tooManyProps:
